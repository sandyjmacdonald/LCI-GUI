...
class App:
    ...
    def build_motor_controls(self):
        """Create coarse and fine motor control frames."""
        ...
        for idx, (txt, d) in enumerate(axes):
            ...
            tk.Button(self.coarse_frame, text=txt, command=lambda r=rel_c: self.move(r)).grid(row=idx//3, column=idx%3, padx=5, pady=5)
            rel_f = (d[0]*self.motor_increment_fine, d[1]*self.motor_increment_fine, d[2]*self.motor_increment_fine)
            tk.Button(self.fine_frame, text=txt, command=lambda r=rel_f: self.move(r)).grid(row=idx//3, column=idx%3, padx=5, pady=5)

    def build_increment_button(self):
        """Create 'Change increments' button."""
        self.change_inc_btn = tk.Button(self.root, text="Change increments", command=self.change_increments)
        self.change_inc_btn.pack(pady=5)

    def change_increments(self):
        """Show dialogs to update fine and coarse increments."""
    ...
    def build_led_control(self):
        ...
        self.led_scale = tk.Scale(led_frame, from_=0.0, to=1.0, resolution=0.01, orient='horizontal', command=self.update_led)
        self.led_scale.set(self.led_brightness)
        self.led_scale.pack(fill='x', padx=10, pady=5)

    def build_preview_button(self):
        """Create external preview toggle."""
        self.preview_btn = tk.Button(self.root, text="Show External Preview", command=self.toggle_external_preview)
        self.preview_btn.pack(pady=5)

    def build_image_display(self):
        """Frame for last captured image."""
        frame = tk.LabelFrame(self.root, text="Last Captured Image", width=400, height=300)
    ...
    def build_timelapse_controls(self):
        ...
        self.start_btn = tk.Button(self.root, text="Confirm settings and start timelapse", command=self.start_timelapse)
        self.start_btn.pack(pady=10)

    def move(self, rel):
        """Move motors without affecting LED."""
        self.sb.move_rel(rel)

    def update_led(self, val):
        """Apply LED slider while previewing."""
        self.led_brightness = float(val)
        if self.previewing:
            self.sb.illumination.cc_led = self.led_brightness

    def toggle_external_preview(self):
        """Start/stop external preview and sync LED."""
    ...
    def start_timelapse(self):
        """Stop preview, disable UI, and capture images."""
        if self.previewing:
            try:
                self.cam.stop_preview()
            except Exception:
        ...
        self.start_btn.config(text="Stop and end timelapse early", command=self.stop_timelapse, state='normal')
        self.timelapse_running = True
        self.capture_loop(frequency)

    def stop_timelapse(self):
        """Stop early and reset UI."""
        if self.after_id:
    ...
    def finish_timelapse(self):
        """Complete timelapse and reset UI."""
        messagebox.showinfo("Done", "Timelapse complete")
        self.reset_controls()

    def reset_controls(self):
        """Re-enable UI after timelapse."""
        ...
        self.start_btn.config(text="Confirm settings and start timelapse", command=self.start_timelapse, state='normal')
        self.timelapse_running = False

    def capture_loop(self, freq):
        """Recursive capture until done."""
